...
class WaveNetTimeRefiner(torch.nn.Module):

    def __init__(self, config):
        super().__init__()
        self.config = config
        self.window = config.window
        self._time_linear = torch.nn.Linear(config.rank, config.rank)
        self.wavenet = WaveNet(layer_size=2, stack_size=2, in_channels=config.rank, res_channels=config.rank)
        self.rainbow = torch.arange(-self.window + 1, 1).reshape(1, -1).to(config.device)
        self.attn = torch.nn.Sequential(torch.nn.Linear(2 * config.rank, 1), torch.nn.Tanh())

    def forward(self, time_embeds):
        # [bs, window, rank]
        outputs = self.wavenet(time_embeds)
        # hs size
        hs_size = outputs.shape[1]
        # query: [bs, 1, rank]
        queries = time_embeds[:, -1, :].unsqueeze(1).repeat(1, hs_size, 1)
        attn = self.attn(torch.cat([outputs, queries], dim=-1))
        time_embeds = torch.sum(outputs * attn, dim=1)
